@extends("dashboard.layouts.app", ["title" => "Tambah Jenis Test"])
    ...
    <div class="py-4 md:pb-7 md:pt-12">
        <div>
            <div class="mb-2 flex items-center gap-2">
                ...
                <p class="text-base font-bold leading-normal text-gray-800 sm:text-lg md:text-2xl lg:text-4xl">Tambah Jenis Test</p>
            </div>
            <p class="text-disabled py-2">Silakan isi data jenis test baru untuk kategori <b>{{ $category->name }}</b>.</p>
        </div>
    </div>
    
    <div class="rounded-[24px] bg-white px-10 py-7">
        <form action="{{ route('dashboard.test-types.store', $category->id) }}" method="POST" class="flex flex-col gap-6">
            @csrf
