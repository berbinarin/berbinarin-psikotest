...
<body>
    <div class="container">
        <div class="header">
            ...
            <a href="{{ route("dashboard.registrants.report.export", $registrant->id) }}" class="mt-8 inline-flex items-start justify-start rounded-lg bg-primary px-6 py-3 text-white hover:bg-primary focus:outline-none focus:ring-2 focus:ring-offset-2 sm:mt-3">
                <span class="leading-none">Export</span>
            </a>
        </div>
        ...
        <div class="section">
            ...
            <div style="background-color: #f0f4f7; padding: 20px; border-radius: 8px;">
                ...
                {{-- Lakukan perulangan untuk setiap hasil tes --}}
                @foreach ($testResults as $result)
                    <h3>Hasil Tes: {{ $result['tool']->name }}</h3>

                    {{-- Sertakan view spesifik untuk setiap hasil tes --}}
                    @if (View::exists("dashboard.ptpm_psikotes-paid.registrants.report.tools" . \Illuminate\Support\Str::slug($result['tool']->name)))
                        @include("dashboard.ptpm_psikotes-paid.registrants.report.tools" . \Illuminate\Support\Str::slug($result['tool']->name), [
                            'tool' => $result['tool'],
                    ...
                    @endif
                    <br>
                    <hr> {{-- Tambahkan garis horizontal sebagai pemisah antar tes --}}
                @endforeach
