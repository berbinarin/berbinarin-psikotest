    ...
    <section class="flex w-full">
        <div class="flex w-full flex-col">
            <div class="py-4 md:pb-7 md:pt-12">
                <div class="mb-2 flex items-center gap-2">
                    <a href="{{ route('dashboard.voucher-code.index') }}">
                        <img src="{{ asset('assets/dashboard/images/back-btn.webp') }}" alt="Back Btn" />
                    </a>
            ...
            <div class="rounded-md bg-white px-4 py-4 shadow-lg shadow-gray-400 md:px-8 md:py-7 xl:px-10">
                <form id="editForm" method="POST" action="{{ route('dashboard.voucher-code.update', $voucher->id) }}">
                    @csrf
                    @method('PUT')
...
@push('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Ambil semua kode voucher yang sudah ada dari backend, kecuali kode voucher yang sedang diedit
            const existingCodes = @json($vouchers->pluck('code')->map(function($code){ return strtolower($code); }));
            const currentCode = "{{ strtolower($voucher->code) }}";

            document.getElementById('editForm').addEventListener('submit', function(e) {
                const codeInput = document.querySelector('input[name="code"]');
                const code = codeInput.value.trim().toLowerCase();

                // Cek duplikat, abaikan kode voucher yang sedang diedit
                if (existingCodes.includes(code) && code !== currentCode) {
                    e.preventDefault();
                    Swal.fire({
                        toast: true,
                        position: "top-end",
