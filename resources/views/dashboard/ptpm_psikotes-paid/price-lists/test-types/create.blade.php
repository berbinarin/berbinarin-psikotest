    ...
    <section class="flex w-full flex-col">
        <div class="py-4 md:pb-7 md:pt-12">
            <div>
                ...
                <p class="text-disabled py-2">Silakan isi data jenis tes psikotes baru pada form berikut.</p>
            </div>
        </div>

        <div class="flex flex-col gap-10 rounded-[24px] bg-white px-10 py-7">
            <form action="{{ route('dashboard.price-lists.test-types.store', $category->id) }}" method="POST" class="flex flex-col gap-10">
                @csrf
                    ...
                    <div class="flex w-full flex-col">
                        ...
                        <select name="test_category_id" id="test_category_id" class="rounded-md border-0 px-6 py-3 text-sm font-semibold drop-shadow-[0_1px_4px_rgba(0,0,0,0.16)] focus:ring-0" required>
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('test_category_id', $category->id) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
