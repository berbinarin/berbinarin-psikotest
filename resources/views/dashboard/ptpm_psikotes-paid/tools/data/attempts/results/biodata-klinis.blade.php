                    ...
                    @if ($option['repeatable'] && $data->responses->form->has($option['group']))
                        @foreach ($data->responses->form->get($option['group']) as $group)
                            @foreach ($option['inputs'] as $input)
                                @php
                                    $value = $group[$input['name']] ?? '-';
                                    $translated = $translateValue($value);
                                @endphp
                                <div class="col-span-{{ $input['span'] ?? 6 }} flex-col gap-1">
                                    <h4 class="font-semibold text-[#9e9e9e]">{{ $input['label'] }}</h4>
                                    <span class="font-semibold">{{ $translated }}</span>
                                </div>
                            @endforeach
                        @endforeach
                    @else
                        @foreach ($option['inputs'] as $input)
                            @php
                                ...
                                if (isset($option['group']) && isset($data->responses->form->get($option['group'])[$input['name']])) {
                                    $rawValue = $data->responses->form->get($option['group'])[$input['name']];
                                } else {
                                    $rawValue = $data->responses->form->get($input['name']) ?? '-';
                                }

                                // Jika select, ambil text dari items
                                if ($input['type'] === 'select' && isset($input['items'])) {
                                    $matchedItem = collect($input['items'])->firstWhere('value', $rawValue);
                                    $displayValue = $matchedItem['text'] ?? $translateValue($rawValue);
                                } else {
                                    $displayValue = $translateValue($rawValue);
                                }
                            @endphp
