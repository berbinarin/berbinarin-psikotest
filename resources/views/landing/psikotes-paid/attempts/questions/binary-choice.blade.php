...
<p class="text-center font-medium text-lg">{{ $question->text }}</p>

<div class="mx-auto flex w-[700px] flex-wrap items-center justify-center gap-x-5 gap-y-6 mt-12">
    @foreach ($question->options as $option)
        <label for="{{ $option['text'] }}" class="card relative flex h-[180px] w-[330px] select-none items-center justify-center rounded-xl bg-[{{ $colors[$loop->index] }}] px-6 transition-all duration-[200ms] hover:scale-[1.03] hover:shadow-[0_8px_16px_rgba(0,0,0,0.25)]">
            <input class="hidden" type="radio" value="{{ $option['value'] ? 1 : 0 }}" id="{{ $option['text'] }}" name="answer" required />
