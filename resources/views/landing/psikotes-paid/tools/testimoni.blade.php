...
@section("content")
    <div class="h-screen overflow-hidden bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset("assets/landing/images/psikotes-paid/psikotes-soal-bg.png") }}')">
        ...
        <!-- Soal Start -->
        <div class="my-7 flex w-full items-center justify-center">
            <h1 id="soal-text" class="w-[395px] text-center text-base font-bold" style="font-family: 'Plus Jakarta Sans', sans-serif">
                <!-- isi awal dari JS -->
            </h1>
        </div>
        <!-- Soal End -->
        ...
        <div class="flex items-center justify-center bg-none">
            ...
            <div class="flex h-[405px] w-[486px] items-start justify-center rounded-xl bg-white pt-10 shadow-md">
                <form class="flex h-full w-full flex-col justify-between px-6 pb-6">
                    <textarea id="testimoni" name="testimoni" rows="6" class="h-[278px] w-full resize-none border-none bg-none px-3 py-2 text-base text-black outline-none placeholder:font-medium placeholder:text-[#D1D1D1]" placeholder="Tulis disini..."></textarea>
                    ...
                    <div id="radio-group" class="ml-[31px] hidden w-full flex-col items-start space-y-4 text-base text-black">
                        <label class="flex cursor-pointer items-center gap-3">
                            <input type="radio" name="setuju" value="Ya" class="h-4 w-4 appearance-none rounded-full border-2 border-black transition duration-200 checked:scale-110 checked:border-blue-600 checked:bg-[#6083F2]" />
                            <span class="text-base font-medium text-black" style="font-family: 'Plus Jakarta Sans', sans-serif">Setuju</span>
                        </label>

                        <label class="flex cursor-pointer items-center gap-3">
                            <input type="radio" name="setuju" value="Tidak" class="h-5 w-5 appearance-none rounded-full border-2 border-black transition duration-200 checked:scale-110 checked:border-blue-800 checked:bg-[#6083F2]" />
                            <span class="text-[15px] font-medium text-black" style="font-family: 'Plus Jakarta Sans', sans-serif">Tidak Setuju</span>
                        </label>
                    </div>
                    <!-- Pilihan Setuju atau Tidak END -->

                    <button type="submit" class="mx-auto mt-4 w-fit rounded-[20px] bg-[#6083F2] px-5 py-[10px] text-base font-bold text-white transition hover:bg-blue-600">Selanjutnya</button>
                </form>
            </div>
            <!-- Form Testimoni End -->
            ...
            <div id="thanks-section" class="hidden flex-col items-center justify-center px-6 text-center">
                ...
                <a href="{{ route('psikotes-paid.tools.index') }}" class="mx-auto w-fit rounded-[20px] bg-[#6083F2] px-5 py-[10px] text-base font-bold text-white transition hover:bg-blue-600">Kembali ke Beranda</a>
            </div>
            <!-- Ucapan Terima Kasih End -->
        </div>

        <script>
            const soal = ['Ceritakan pengalaman SobatBinar dalam mengikuti kegiatan psikotes di Berbinar!', 'Bagaimana pendapat SobatBinar mengenai kegiatan psikotes yang telah terlaksana?', 'Apakah SobatBinar memiliki masukan atau saran mengenai pelaksanaan psikotes di Berbinar?', 'Apa SobatBinar setuju jika Berbinar membagikan testimoni SobatBinar melalui media sosial kami?'];
