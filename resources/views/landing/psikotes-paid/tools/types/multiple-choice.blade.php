...
<div class="mx-auto mt-12 flex w-[700px] flex-wrap items-center justify-center gap-x-5 gap-y-6">
    @foreach ($question->options as $option => $text)
        <label for="{{ $option }}" class="card relative flex h-[180px] w-[330px] select-none items-center justify-center rounded-xl bg-[{{ $colors[$loop->index] }}] px-6 transition-all duration-[200ms] hover:scale-[1.03] hover:shadow-[0_8px_16px_rgba(0,0,0,0.25)]">
            <input class="hidden" type="radio" value="{{ $option }}" id="{{ $option }}" name="answer" value="{{ $option }}" required />
            <span  class="text-center text-sm font-semibold text-white">{{ $text }}</span>
